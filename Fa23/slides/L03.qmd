...
- We assume that the two groups come from different populations where
    ...
    - $\mu_i$ is the mean for group $i$; $\bar{x}_i$ estimates $\mu_i$,
    
    - $\sigma^2_i$ is the standard deviation for group $i$; $s^2_i$ estimates $\sigma^2_i$
    
    - $N_i$ is the population size of group $i$; $n_i$ is the sample size for group $i$. 
