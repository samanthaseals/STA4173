...
- We can use base R for some things, but I try to stay in the [`tidyverse`](https://www.tidyverse.org/) when possible.
...
```{r, eval = FALSE}
dataset %>%
  group_by(var1, var2) %>% # do not change anything under this
  summarise(n = n()) %>%
  mutate(freq = n / sum(n))
```
...
- If we want conditional probabilities, we need to filter to one level.

    - Of automatic cars, 15 (78.9%) have 3 forward gears, while 4 (21.1%) have 4 forward gears.
...
- If we want conditional probabilities, we need to filter to one level.
    ...
    - Of manual cars, 8 (61.5%) have 4 forward gears, while 5 (38.5%) have 5 forward gears.
