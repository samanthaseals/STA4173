...
# view results through summary() function
summary(m1)

# check for outliers
# this creates a variable that contains TRUE/FALSE values
# it will be TRUE if the absolute value of the Studentized residual is larger than 2.5
# it will be FALSE otherwise
data$outlier <- abs(rstandard(m1))>2.5

# sensitivity analysis -- look at model without outliers

# filter to only those NOT classified as an outlier
data2 <- filter(data, outlier == FALSE)

# model of interest, without outliers
