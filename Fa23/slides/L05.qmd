...
- Let's look at using the `aov()` and `summary()` functions.
...
m3 <- aov(bill_length_mm ~ species, data = penguins)
summary(m3)
```
## Example - Penguins
...
```{r, echo = TRUE}
m4 <- aov(bill_length_mm ~ species, data = penguins)
summary(m4)
```
...
```{r}
results <- summary(m4)
LSD.test(penguins$bill_length_mm, 
         penguins$species, 
         results[[1]]$Df[2], 
         results[[1]]$`Mean Sq`[2])[5]
```
